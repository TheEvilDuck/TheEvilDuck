...
local Status = require(script.Parent.Parent:WaitForChild("LocalStatus"))
...
function ControlModule:OnRenderStepped(dt)
	if self.activeController and self.activeController.enabled and self.humanoid then
		...
		game.ReplicatedStorage.Events.ChangedDirectionOfMovement:Fire()
		...
		if self.activeController ~= clickToMoveController then
			if moveVector.magnitude > 0 then
				...
				moveVector = Status.GetMoveVector()
		...
		--DISABLE MOVEMENT
		if not Status.Rolling
			and not (Status.Attacking and not Status.Swinging)
		then
		...
		if FFlagUserFixExternalJumpRequest then
			...
			self.humanoid.Jump =(self.activeController:GetIsJumping() or (self.touchJumpController and self.touchJumpController:GetIsJumping())) and not(Status.Rolling or Status.Attacking) 
		end
	end
end
